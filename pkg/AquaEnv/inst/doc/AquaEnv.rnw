...
ae <- aquaenv(S, t, p, SumCO2=SumCO2, pCO2=pCO2, dsa=TRUE, revelle=TRUE)
ae$dTAdH
ae$revelle
@ 
...
fit <- TAfit(aquaenv(S=33.923, t=24.25), sample, 
...
testtit <- titration(aquaenv(S=33.923, t=24.25, SumCO2=0, TA=0.00226006, SumBOH3=0), mass_sample=mass_sample,
...
<<TAfitDickson2007_02, fig=FALSE>>=
testtotpH <- convert(testtit$pH, "pHscale", "free2tot", S=33.923, t=24.25)
testE  <- -(-0.394401 - (((PhysChemConst$R/10)*testtit$T[[1]])/PhysChemConst$F)*log(10^(-testtotpH)))
@ 
...
Upt to now, it is unclear where this offset comes from, this is something to be looked into.
...
salinity \texttt{S} and temperature \texttt{t} from the above references are pressure corrected using the given depth \texttt{d} and the calculated hydrostatic pressure \texttt{hydroP} according to \citet{Millero1995} 
