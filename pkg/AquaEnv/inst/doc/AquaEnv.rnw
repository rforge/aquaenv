...
\texttt{Tc}          & \textdegree C                & temperature          \\
\texttt{Tk}          & K                            & absolute temperature \\
\texttt{S}           & ``psu'' (no unit)            & salinity             \\
\texttt{Cl}          & \textperthousand             & chlorinity           \\
\texttt{I}           & mol/kg-$\rm H_2O$            & ionic strength       \\
\texttt{d}           & m                            & depth                \\ 
\texttt{hydroP}      & bar                          & hydrostatic pressure \\
...
\texttt{dTAdKdKdT}   & -                               & $\rm \sum_i \frac{\partial [TA]}{\partial K^*_i} \frac{\partial K^*_i}{\partial T}$\\
            &                                 & with $\rm [TA] = f([H^+], [\sum CO_2], ..., K^*_i)$\\     
\texttt{dTAdKdKdd}   & -                               & $\rm \sum_i \frac{\partial [TA]}{\partial K^*_i} \frac{\partial K^*_i}{\partial d}$\\
            &                                 &  with $\rm [TA] = f([H^+], [\sum CO_2], ..., K^*_i)$\\ 
...
intersect at \texttt{S}=5. The true intersection with respect to salinity \texttt{S} is a function of temperature \texttt{Tc}. Here, we first calculate this intersection by
...
\texttt{dTAdH},  \texttt{dTAdSumCO2}, \texttt{dTAdSumBOH3}, \texttt{dTAdSumH2SO4}, \texttt{dTAdSumHF}   & \citet{Hofmann2008}\\
\texttt{c1}, \texttt{c2}, \texttt{c3},  \texttt{b1}, \texttt{b2}, \texttt{so1}, \texttt{so2}, \texttt{so3}, \texttt{f1}, \texttt{f2} & \citet{Skoog1982}, \citet{Stumm1996}, \citet{Hofmann2008c}\\
\texttt{dTAdKdKdS}, \texttt{dTAdKdKdT}, \texttt{dTAdKdKdd}, \texttt{dTAdKdKdSumH2SO4}, \texttt{dTAdKdKdSumHF} & \citet{Hofmann2008b}\\ \hline
\end{longtable}
\end{footnotesize}
\noindent
...
salinity \texttt{S} and temperature \texttt{Tc} from the above references are pressure corrected using the given depth \texttt{d} and the calculated hydrostatic pressure \texttt{hydroP} according to \citet{Millero1995} 
