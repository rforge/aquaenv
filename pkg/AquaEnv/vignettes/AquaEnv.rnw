...
\Abstract{ 
\noindent
\code{AquaEnv} \citep{AquaEnv} is an integrated development toolbox for aquatic chemical model generation focused on (ocean) acidification and CO2 air-water exchange.  It contains
\begin{itemize}
\item All elements necessary to model the pH, the related CO2 air-water exchange, as well as aquatic acid-base chemistry in general for
an arbitrary marine, estuarine or freshwater system. 
...
\item a routine to determine total alkalinity ([TA]), the total dissolved inorganic carbon concentration ([$\sum$CO2]), 
...
\code{AquaEnv} can be used in two ways
\begin{itemize}
\item It provides separate functions to calculate the stoichiometric equilibrium constants ($\rm K^*$) for key acid base systems in natural seawater, the Henry's constants ($\rm K_0$), 
      as well as the solubility products ($\rm K_{sp}$) for calcite and aragonite. 
      
      This functionality is provided via the functions 
      \code{K\_CO2, K\_HCO3, K\_BOH3, K\_W, K\_HSO4, K\_HF, K\_NH4, K\_H2S, K\_H3PO4, K\_H2PO4, K\_HPO4, K\_SiOH4, K\_SiOOH3, K0\_CO2, K0\_O2, Ksp\_aragonite,} and \\
      \code{Ksp\_calcite}.

\item It is also possible to estimate all the acid-base chemistry using one single function: \code{aquaenv}. This function returs a list of class  \emph{aquaenv} that contains, in addition to the input parameters: 
\begin{itemize}
...
\footnote{Please note that if values for $[\rm \sum B(OH)_3]$, $[\rm \sum H_2SO_4]$, $[\rm \sum HF]$ are given as input parameters, these parameters are used and not the ones calculated from salinity.}
...
\item if the flag \code{revelle=TRUE} is set, the revelle factor \citep{Zeebe2001} is calculated.
item if the flag \code{revelle=TRUE} is set, all necessary quantities for the explicit ``direct substitution approach'' (DSA) to pH modelling as given in \cite{Hofmann2008} are 
...
\end{itemize}
\item Input for \code{aquaenv} has to be supplied in standard SI units, the free proton pH scale and in molinity\footnote{Note that it is not sufficient to give a gravimetric concentration in 
...
Conversion of input parameters to this necessary units and pH scale  can be done with the generic function \code{convert}.
\item The information created with aquaenv is also supplied in standard SI units and in molinity. All elements of an object of class \code{aquaenv} of a certain unit or pH scale
...
the input vector. This can be visualized in a large variety of ways using the \code{plot} function specially defined for objects pf class \code{aquaenv}.
...
with the function \code{aquaenv} and the flag \code{from.data.frame=TRUE} it is possible to convert output of those dynamic models into objects pf class
...
\item \cite{Hofmann2008}, \cite{Hofmann2009}, and \cite{Hofmann2008c} describe methods for an ``explicit'' pH modelling which allows 
for the quantification of the influences of kinetically modelled processes on the pH. Objects pf class \code{aquaenv} provide all needed quantities 
(partial derivatives of $\rm[TA]$, ionization fractions, etc.) to employ both of those methods in dynamic models. 
\aq$\,$ also provides the functionality to cumulatively plot the obtained influences on the pH.
\item As an example of how to use the aquatic chemical toolbox that is provided by \aq, two applications are provided
\begin{itemize}
\item The function \code{titration}: creates theoretical titrations which can be used e.g. to create bjerrum plots with the function \code{plot.aquaenv} in \aq.
...
\section{The elements of an object of class \code{aquaenv}}
...
\code{skeleton} is \code{FALSE}, an object of class \code{aquaenv} contains the following elements \footnote{Literature references are in appendix \ref{app: references}.
}
...
\code{K\_W}        &(mol/kg-soln)$^2$, free pH scale & stoichiometric equilibrium ion product of \\
...
\code{K\_HS}       & mol/kg-soln; mol/kg-H2O; mol/l  & approximate value for equilibrium constant \\
            &                                 & $\rm K^*_{HS^-} = [H^+][S^{2-}] / [HS^-]$\\
\code{Ksp\_calcite}& (mol/kg-soln)$^2$               & stoichiometric equilibrium solubility product of calcite\\
            &                                 & $\rm Ksp^*_{cal} = [Ca^{2+}][CO_3^{2-}]$\\
\code{Ksp\_aragonite} &(mol/kg-soln)$^2$             & stoichiometric equilibrium solubility product of aragonite\\
            &                                 & $\rm Ksp^*_{ara} = [Ca^{2+}][CO_3^{2-}]$\\
\code{TA}          & mol/kg-soln                     & [TA], total alkalinity\\
\code{pH}          & -, free scale                   & pH\\              
\code{fCO2}        & atm,                            & fugacity of $\rm CO_2$ in the water (i.e. in a small volume of air equilibrated with the water)\\
...
\code{H2PO4}       & mol/kg-soln                     & $[\rm H2PO_4^-]$\\                    
...
\section{Using \aq}
\subsection{calling the ``K'' functions directly}
The elements \code{K\_CO2, K\_HCO3, K\_BOH3, K\_W, K\_HSO4, K\_HF, K\_NH4, K\_H2S, K\_H3PO4, K\_H2PO4, K\_HPO4, K\_SiOH4, K\_SiOOH3, K0\_CO2, K0\_O2, Ksp\_aragonite,} and \code{Ksp\_calcite} can be calculated directly. This is done via functions that bear the same name as those elements
...
\subsection{The function \code{aquaenv}}


\subsubsection{Minimal \code{aquaenv} definition}
...
Optionally the pressure (here the gauge pressure) can be given. As in the above case, the returned object of class \code{aquaenv} then contains
...
\subsubsection{Defining the complete \code{aquaenv} system in different ways}
...
All the quantities needed for the explicit pH modelling approaches as given in \cite{Hofmann2008} and \cite{Hofmann2008c} can be calculated by 
setting the flag \code{dsa} to \code{TRUE}. 
MATHILDE????
...
\subsubsection{Cloning an object of class \code{aquaenv}}

 It is possible to clone an obect of class \code{aquaenv}, either 1 to 1 or with different pH, TA, or K\_CO2
...
Note that K\_CO2 as an input argument is in lower case!
...
Data in other concentration units or pH scales can be converted using the function \code{convert}.

This is a rather complex function, but its typical use is as:
\begin{verbatim}
...
But also SumCO2, TA, pH and SumNH4 can be vectors, e.g. 
...
ae <- aquaenv(S = 30, t = 11:15, SumCO2 = NULL, pH = pH, CO2 = CO2, dsa = TRUE)
ae$SumCO2
@ 
...
\subsubsection{Converting  elements in an object of class \code{aquaenv}}
...
can be calculated with the function \code{aquaenv} by setting the flag \code{dsa}.

<<DSAQuantities1, fig=FALSE, echo=TRUE>>=
ae <- aquaenv(S = 30, t = 15, d = 10, SumCO2 = 0.002, pH=8, dsa = TRUE)
@ 

This command calculates the buffer factor and the partial derivatives of [TA] with respect to other summed quantities referred to in \cite{Hofmann2008}
MATHILDE????
...
\subsection{The \code{plot.aquaenv} function}
...
Furthermore, the function \code{plot.aquaenv} can be used to create ``cumulative'' plots and ``Bjerrum'' plots. This will be explained in the following sections.


\subsection{Using objects of class \code{aquaenv} in dynamic models}
...
The model is solved, and the output can be plotted in the same way as any deSolve object.
...
And a model function is defined. Again, an object of class \code{aquaenv} is created in each timestep and respective elements are used.
...
@ 
\end{scriptsize}
visualisation can be done similar as above.
...
\subsection{Titration simulation: the function \code{titration}}
...
\subsection{Calculating information from titration curves: the function \code{TAfit}}

\subsubsection{A little theory}
While titrating a sample of natural seawater with HCl one sees two clear equivalence points \citep{Dickson1981}
...
par(new = TRUE)
plot(ae$delta_mass_titrant, prot1, type = "l", col = "blue", xlim = c(0,0.5), 
     ylab = "", xlab = "", yaxt = "n", ylim = c(-0.015, 0.015))
prot2 <- c()
for (i in 1:length(ae$pH))
    prot2 <- c(prot2, (10^-(ae$pH[[i]])+ae$HSO4[[i]]+ae$HF[[i]]-
               ae$HCO3[[i]]-2*ae$CO3[[i]]-ae$BOH4[[i]]-ae$OH[[i]]))
par(new = TRUE)
plot(ae$delta_mass_titrant, prot2, type = "l", col = "green", xlim = c(0,0.5), 
     ylab = "", xlab = "", yaxt = "n", ylim = c(-0.015,0.015))
...
@ 
Note that For the \code{TAfit} all total quantities except SumCO2 (SumNH4, SumH2S, SumH3PO4, SumSiOH4, SumHNO3, SumHNO2, SumBOH3, SumH2SO4, SumHF)
...
fit1
@ 
Thus, we see that \code{TAfit} calculates the correct SumCO2 and TA values.\\
...
Etitcurve   <- cbind(ae$delta_mass_titrant, 
  (0.4 - ((PhysChemConst$R/10)*initial_ae$T/PhysChemConst$F)*log(10^-tottitcurve[,2]))) 
@ 
...
\subparagraph{Does the salinity correction (\code{S\_titrant}) matter?}$\;$\\
...
\section{References for the elements of an object of class \code{aquaenv}} \label{app: references}
\begin{footnotesize}
...
\code{revelle}     & \citet[p.73]{Zeebe2001}\\   
